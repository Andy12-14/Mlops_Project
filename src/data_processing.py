...
def process_dataframe(
    ...
    Args:
        ...
        reply_column: Name of the reply text column to process.
    ...
    text_cols = [col for col in [text_column, reply_column] if col in df.columns]
    df = df[text_cols].copy()
    
    # Clean text columns
    df[f'cleaned_{text_column}'] = df[text_column].apply(clean_text)
    if reply_column in df.columns:
        df[f'cleaned_{reply_column}'] = df[reply_column].apply(
...
if __name__ == '__main__':
    ...
    print(results['train_df'][['cleaned_content']].head())